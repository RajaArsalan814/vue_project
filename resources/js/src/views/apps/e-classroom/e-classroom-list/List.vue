  ...
  <div>
    ...
    <b-card no-body class="mb-0">
          ...
          <b-col cols="12" md="6">
            <div class="d-flex align-items-center justify-content-end">
              ...
              <b-button variant="primary" @click="isAddNewUserSidebarActive = true">
                <span class="text-nowrap">Add eClassroom</span>
              </b-button>
            </div>
          </b-col>
        </b-row>
      ...
      <b-table ref="refUserListTable" class="position-relative" :items="fetchUsers" responsive :fields="tableColumns"
        ...
        <template #cell(ct_activity)="data">
          <b-badge pill variant="primary" class="cursor-pointer"  
              @click="$router.push({ name: 'classroom-activity', params: { id: data.item.id } })">
              {{data.item.ct_activity}}
            </b-badge>
        </template>
        ...
        <template #cell(actions)="data">
          <div class="text-nowrap">
            <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`"  icon="PlusIcon" size="16" class="mx-1 cursor-pointer"
              @click="$router.push({ name: 'classroom-activity', params: { id: data.item.id } })" />
            <b-tooltip title="Add Activity or Game" :target="`invoice-row-${data.item.id}-preview-icon`" />
