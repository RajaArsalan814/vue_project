  ...
  <b-sidebar id="add-new-user-sidebar" :visible="isAddNewUserSidebarActive" bg-variant="white"
    ...
    <template #default="{ hide }">
      ...
      <div class="
          ...
          py-1
        ">
        <h5 class="mb-0"> {{ editData.id ? 'Update' : 'Add ' }} Lesson</h5>
      ...
      <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
        ...
        <b-form class="p-2" @submit.prevent="handleSubmit(onSubmit)" @reset.prevent="resetForm">
          ...
          <validation-provider #default="validationContext" name="title" rules="required">
            <b-form-group label="Lesson Name" label-for="title">
          ...
          <validation-provider #default="validationContext" name="Module" rules="required">
            <b-form-group label="Select Module" label-for="module_id" :state="getValidationState(validationContext)">
              <v-select v-model="editData.module_id" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                :options="glsdata" :reduce="(val) => val.id" label="title" value="id" :clearable="false"
                input-id="module_id" />
          ...
          <!-- <validation-provider #default="validationContext" name="Component Type" rules="required">
            <b-form-group label="Component" label-for="component" :state="getValidationState(validationContext)">
              <v-select v-model="editData.component" :dir="$store.state.appConfig.isRTL ? 'rtl' : 'ltr'"
                :options="roleOptions" :reduce="(val) => val.value" :clearable="false" input-id="component" />
              <b-form-invalid-feedback :state="getValidationState(validationContext)">
                {{ validationContext.errors[0] }}
              </b-form-invalid-feedback>
            </b-form-group>
  ...
  methods: {
    getCategories() {
      store
        ...
        .then(response => {
          this.glsdata = response.data.module
        })
    }
  },
  mounted() {
